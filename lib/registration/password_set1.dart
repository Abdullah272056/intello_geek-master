...
class PasswordSetScreen1 extends StatefulWidget {
   String name,surName,email,mobile,countryId;
   // String surName;
   // String email;
   // String mobile;
   // String countryId;
   ...
   PasswordSetScreen1({required this.name, required this.surName, required this.email, required this.mobile,
      required this.countryId}); //  PasswordSetScreen({Key? key}) : super(key: key);

  @override
  State<PasswordSetScreen1> createState() => _PasswordSetScreenState(
      this.name, this.surName, this.email, this.mobile,
      this.countryId);
}

class _PasswordSetScreenState extends State<PasswordSetScreen1> {
  String name,surName,email,mobile,countryId;
  _PasswordSetScreenState(this.name, this.surName, this.email, this.mobile,
      this.countryId);
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          setState(() {
              _userRegistration(
                //  String name,surName,email,mobile,countryId;
                  name: name,surName: surName,email: email,mobile: mobile,countryId:countryId,password:confirmPassword
              );
            });
        ...
        try {
          ...
          await post(Uri.parse('$BASE_URL_API$SUB_URL_API_REGISTRATION'),
              body: {
                ...
                'receive_marketing_email_update': 'true',
          ...
          if (response.statusCode == 200) {
            ...
            setState(() {
              ...
              Navigator.push(context,MaterialPageRoute(builder: (context)=>SendOtpForVerifyScreen(userId: data['data']['id'].toString(),)));
            ...
            //   context,
            //   MaterialPageRoute(
            //     builder: (BuildContext context) => NavigationBarScreen(0,HomeScreen()),
            //   ),
            //       (route) => false,
            // );
