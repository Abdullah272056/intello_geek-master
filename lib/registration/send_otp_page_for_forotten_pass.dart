...
class SendOtpForForgottenPasswordVerifyScreen extends StatefulWidget {
   String userId;
   // String surName;
   // String email;
   // String mobile;
   ...
   SendOtpForForgottenPasswordVerifyScreen({required this.userId}); //  PasswordSetScreen({Key? key}) : super(key: key);

  @override
  State<SendOtpForForgottenPasswordVerifyScreen> createState() => _SendOtpForForgottenPasswordVerifyScreenState(this.userId);
}

class _SendOtpForForgottenPasswordVerifyScreenState extends State<SendOtpForForgottenPasswordVerifyScreen> {
  String _userId;
  _SendOtpForForgottenPasswordVerifyScreenState(this._userId);
  ...
  _userVerify(
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        try {
          Response response =
          await post(Uri.parse('$BASE_URL_API$SUB_URL_API_FORGET_PASSWORD_VERIFY'),
              body: {
                'user_id': userId,
                'code': otp,
              });
          ...
          if (response.statusCode == 200) {
            ...
            setState(() {
              ...
              Navigator.push(context,MaterialPageRoute(builder: (context)=>
                  PasswordSetScreen(userId: _userId),));
            });
